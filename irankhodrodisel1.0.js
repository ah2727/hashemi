...
(function() {
  ...
  const defaultnumber='09017670855'
  //login
  let token = ""
...
*/
  ...
  async function getMessage(mobileNumber){
      ...
      const lastSmsResponse = await axios.get(`https://khodro.bot1234.online/api/last-sms/${mobileNumber}`);
    ...
    <div class="popup-content">
      ...
      itemsBox.addEventListener('click', async (event) => {
          ...
          if (event.target && event.target.matches('.item')) {
              const filteredItems = items.saleProjects.filter(item => item.Id === event.target.id);
              ...
              datainit =await OrderInit(filteredItems[0])
      ...
      smsButton.addEventListener('click',async () => {
          ...
          setTimeout(async () => {
              const smsAnswer = await getMessage(mobileNumber);

              // Use a regular expression to extract the numeric code
              const codeMatch = smsAnswer.match(/\d+/); // Matches the first sequence of digits
              const numericCode = codeMatch ? codeMatch[0] : "Code not found"; // Get the first match or fallback

              // Set the extracted code into the input field
              smsInput.value = numericCode;
          }, 3000); // 3-second delay
